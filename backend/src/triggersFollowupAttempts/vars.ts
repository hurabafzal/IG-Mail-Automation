...
export function getInstantlyVariables(ig_id: string) {
	return Effect.gen(function* () {
		...
		return {
			...
			subject: `Kooperationsanfrage für @${accountDetails.username}`,
		};
	});
}

/**
 * 🔄 Updates the custom variables for a lead in Instantly using InstantlyLead data
 *
 ...
 *
 * @param lead - The lead object from Instantly API containing current payload
 * @param email_id - The Email table ID to get Instagram account ID
 * @returns Effect representing the update operation
 */
export function updateInstantlyVariables(lead: Lead, email_address: string) {
	return Effect.gen(function* () {
		// 📧 Get Instagram ID from Email table
		...
		if (!email) {
			return yield* Console.warn(
				`[vars] Email with ID ${email_address} not found for lead ${lead.id}`,
			);
		}

		// 📥 Fetch latest variables for the Instagram account
		const vars = yield* getInstantlyVariables(email.instagram_id);
		...
		if (!deepEqual(updatedPayload, lead.payload)) {
			yield* Console.log(
				`[vars] Updating Instantly variables for lead ${lead.id} (IG: ${email.instagram_id})`,
			);
			yield* Console.log(
				`[vars] Diff: ${JSON.stringify(deepDiff(updatedPayload, lead.payload))}`,
			);
			...
			if (res.status !== 200) {
				...
				yield* Console.error(
					`[vars] Failed to update lead ${lead.id} variables: ${res.status} - ${text}`,
				);
			} else {
				yield* Console.log(
					`[vars] Successfully updated variables for lead ${lead.id}`,
				);
			}
		}
	}).pipe(
