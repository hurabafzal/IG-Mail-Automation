 ...
 */
export function runEmailGenerationWorkerKeywordspipedrive(batchSize: number) {
	return Effect.gen(function* () {
		// ✅ Hardcoded country tasks instead of DB
		const activeFilterCloneTasks = [
			{ id: 200, country: "Germany", is_active: true },
			{ id: 300, country: "Netherlands", is_active: true },
			{ id: 400, country: "United Kingdom", is_active: true },
			{ id: 500, country: "United States", is_active: true },
		];

		for (const task of activeFilterCloneTasks) {
			if (!task.is_active) continue; // safety

			console.log(`Active Country: ${task.country} (id: ${task.id})`);
			console.log("🤖 Starting Email Generation Worker");

			const emailRequests = yield* getGenerationRequestData(
				batchSize,
				Number(task.id), // still pass id for processing
			);
			console.log(`Found ${emailRequests.length} email requests to process`);
