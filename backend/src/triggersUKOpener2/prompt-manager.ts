...
interface EmailContext {
	...
	stageNumber: number;
	startviralPackage: StartviralPackage;
	mostEngagingPost: {
		caption: string | null;
...
DO NOT include formal closings like "Best regards". Use a conversational closing appropriate for the language:
- German: "Liebe Grüße"
- English: "Best regards"
- Dutch: "Met vriendelijke groet"
Use the correct closing for the language of the email.
...
function buildOpenAIRequestPayload(
	...
	return Effect.gen(function* () {
		...
		const baseContext: EmailContext = {
			...
			externalLink: account.external_link,
			stageNumber: 1, // Not used anymore but keeping for interface compatibility
			startviralPackage,
			mostEngagingPost: mostEngagingPost
				? {
						caption: sanitizeAndTruncate(mostEngagingPost.caption, 1000),
						likeCount: mostEngagingPost.like_count,
						commentCount: mostEngagingPost.comment_count,
						takenAt:
							mostEngagingPost.taken_at instanceof Date
								? mostEngagingPost.taken_at.toISOString()
								: mostEngagingPost.taken_at,
						thumbnail: mostEngagingPost.thumbnail,
						thumbnailBase64: mostEngagingPost.thumbnail
							? imageMap.get(mostEngagingPost.thumbnail)
							: undefined,
					}
				: null,
			recentPosts: recentPosts.slice(0, 5).map((post) => ({
				caption: sanitizeAndTruncate(post.caption, 1000),
				likeCount: post.like_count,
				commentCount: post.comment_count,
				takenAt:
					post.taken_at instanceof Date
						? post.taken_at.toISOString()
						: post.taken_at,
				thumbnail: post.thumbnail,
				thumbnailBase64: post.thumbnail
					? imageMap.get(post.thumbnail)
					: undefined,
			})),
		};
...
async function buildOpenerEmailPayload(
	...
	const systemPrompt = await startviralOpenerEmailSystemPromptLanguage(
		"",
		language,
	);
 ...
 */
function buildTriggerEmailPayload(
	context: EmailContext,
	emailType: GeneratedEmailType,
	...
	if (emailType === GeneratedEmailType.TRIGGER_FOLLOWER_LOSS) {
		triggerContext = "Recent follower loss detected";
		specificInstructions =
			"Address the follower loss empathetically and explain how Startviral can help recover and grow with quality, engaged followers. Focus on how losing followers might indicate they need more targeted, quality growth.";
	} else if (emailType === GeneratedEmailType.TRIGGER_FOLLOWING_INCREASE) {
		triggerContext = "Increased following activity detected";
		specificInstructions =
			"Reference their increased activity in following others and suggest it's a good time to focus on growing their own audience with targeted campaigns. Position this as smart timing for growth.";
	} else if (emailType === GeneratedEmailType.TRIGGER_HIDDEN_LIKES) {
		triggerContext = "Hidden likes on posts detected";
		specificInstructions =
			"Acknowledge the hidden likes strategy and explain how Startviral can help improve overall engagement beyond just like counts. Focus on comprehensive engagement metrics.";
	}
	const language = getMainLanguageByCountry(context.country);
	const systemPrompt = startviralTriggerEmailSystemPromptLanguage("", language);
	...
	const userContent: Array<{
		...
		{
			type: "text",
			text: `Generate a trigger follow-up email for @${context.username}.

Trigger context: ${triggerContext}
Approach: ${specificInstructions}
...
Startviral package for their follower tier:
- Daily budget: €${context.startviralPackage.dailyAdBudget}
- Monthly budget: €${context.startviralPackage.monthlyAdBudget}
- Expected followers: ${context.startviralPackage.expectedFollowerGrowthMin}-${
				context.startviralPackage.expectedFollowerGrowthMax
			}/month
- Expected story views: ${
				context.startviralPackage.expectedStoryViewGrowthMin
			}-${context.startviralPackage.expectedStoryViewGrowthMax}/day

${
	context.mostEngagingPost
		? `Example post for ad creative:
Caption preview: ${
				sanitizeUnicode(context.mostEngagingPost.caption?.substring(0, 150)) ||
				"No caption"
			}...
Engagement: ${context.mostEngagingPost.likeCount} likes, ${
				context.mostEngagingPost.commentCount
			} comments`
		: "No recent posts available for ad creative examples"
}

Generate the trigger email addressing the specific behavioral change and how Startviral can help with their current growth situation.`,
		},
	];
	...
	// TODO: maybe add recent posts images? not sure
 ...
 */
function fetchPromptDataForSequence(sequence: Selectable<EmailSequence>) {
	return Effect.gen(function* () {
...
export function fetchPromptDataForSequences(
	...
	return Effect.gen(function* () {
		...
		for (const sequence of sequences) {
			const result = yield* fetchPromptDataForSequence(sequence);
