 ...
 */
export function runEmailGenerationWorkerUK(batchSize: number) {
	return Effect.gen(function* () {
		console.log("🤖 Starting Email Generation Worker");

		// Process pending sequences
		const emailRequests = yield* getGenerationRequestData(batchSize);
 ...
 */
function getGenerationRequestData(batchSize: number) {
	return Effect.gen(function* () {
		// Get sequences pending generation
		const sequencesToProcess = yield* getSequencesPendingGeneration(batchSize);
		...
		const promptDataList =
			yield* fetchPromptDataForSequences(sequencesToProcess);
...
function getSequencesPendingGeneration(batchSize: number) {
	...
	return (
		db
			...
			.selectAll("EmailSequence")
			.where("current_stage", "in", pendingGenerationStages)
			.where("current_stage_number", "=", 4) // <-- Only stage 4
			.where("next_action_possible_at", "<=", new Date())
			// .where("last_data_refreshed_at", "is not", null)
			// .where(
			// 	"last_data_refreshed_at",
