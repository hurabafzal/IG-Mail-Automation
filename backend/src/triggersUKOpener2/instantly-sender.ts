...
const instantlyCampaigns = {
	...
	UK: {
		openers: {
			1: "c9059462-511e-4ab3-a04d-6cebba16be1d", // 🇬🇧 Campaign 1
			2: "c9059462-511e-4ab3-a04d-6cebba16be1d", // 🇬🇧 Campaign 2
			3: "",
		},
		triggers: {
			1: {
 ...
 */
export function runCampaignSubmissionWorkerUK() {
	return Effect.gen(function* () {
...
function getCountryCampaignRegion(
	...
	if (!country) return "UK"; // Default to German campaigns

	return countryToCampaignRegion[country] || "UK"; // Default to German campaigns for unknown countries
}
...
function getCampaignIdForSequence(
	...
	return "c9059462-511e-4ab3-a04d-6cebba16be1d";
...
function getSequencesReadyForCampaign() {
	...
	return db
		...
		.where("EmailSequence.current_stage_number", "=", 4)
					...
					results.map((row) => {
						...
						return {
							...
							account: {
								...
								country: row.country || "United Kingdom", // Default to United Kingdom if not set
							},
						};
...
//         console.log(`Input: "${name}" → Output: "${capitalizeName(name as any)}"`);
//     }
// }
