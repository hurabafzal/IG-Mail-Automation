...
// German campaign configuration (different from English system)
const instantlyCampaigns = {
	DE: {
		openers: {
			1: "f5f0b5a4-8018-4a49-b909-efef161337cf", // 🇩🇪 German Campaign 1
			2: "",
			3: "",
		},
		triggers: {
	...
	return Effect.gen(function* () {
		...
		for (const sequenceData of limitedSequences) {
			...
			if (result._tag === "Right") {
				...
				console.error(
					`Failed to process sequenc ${sequenceData.sequence.id}:`,
					result.left,
				);
			}
		}
...
/**
 * Main dynamic lead management worker for German system - Smart campaign-aware lead management
 */
export function runDynamicLeadManagerDE(): Effect.Effect<void, Error> {
	return Effect.gen(function* () {
		yield* Console.log("🚀 Starting Dynamic Lead Manager DE");
		...
		for (const [campaignId, dailyLimit] of Object.entries(campaignLimits)) {
			...
			yield* Console.log(
				`[Campaign DE ${campaignId.slice(0, 8)}...] Current: ${currentCount}, Optimal: ${optimalCount}, Added: ${added}`,
			);
		}

		// 5. Send Slack notification only if leads were added
		// if (totalAdded > 0) {
		// 	yield* sendSlackMessageE(
		// 		`[Dynamic Lead Manager DE] Added ${totalAdded} leads:\n• Campaigns Managed: ${Object.keys(campaignLimits).length}\n• Campaign Details:\n${campaignResults.map((r) => `  • ${r}`).join("\n")}`,
		// 	);
		// }

		yield* Console.log("✓ Dynamic Lead Manager DE cycle complete");
	}).pipe(
		Effect.catchAll((e) => {
			console.error("Error in dynamic lead manager DE:", e);
