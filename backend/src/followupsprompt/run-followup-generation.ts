...
export function runFollowupGeneration() {
	return Effect.gen(function* () {
		...
		// Constants for follow-up attempt date fields
		...
		// Get today's date in Germany timezone (Europe/Berlin) - calculate once for all deals
		...
		// Process each deal - check dates BEFORE fetching expensive data
		for (const dealInfo of dealsNeedingProcessing) {
			// Step 1: Check if this is a follow_up deal by stage_id (17 = follow_up_1, 18 = follow_up_2)
			const isFollowUpStage =
				dealInfo.deal.stage_id === 17 || dealInfo.deal.stage_id === 18;

			// If it's a follow_up stage, check dates BEFORE fetching expensive data
			if (isFollowUpStage) {
				const attempt1Date = (dealInfo.deal as Record<string, unknown>)[
					FOLLOW_UP_ATTEMPT_1_FIELD
				] as string | undefined;
				const attempt2Date = (dealInfo.deal as Record<string, unknown>)[
				...
				console.log(
					`📅 Date check for deal ${dealInfo.deal.id} (BEFORE fetch): Attempt 1: ${attempt1Date || "not set"} ${attempt1IsToday ? "✅" : "❌"}, Attempt 2: ${attempt2Date || "not set"} ${attempt2IsToday ? "✅" : "❌"}, Today (Germany): ${todayInGermanyStr}`,
				);

				// If neither date matches today, skip this deal entirely (no fetch, no processing)
				if (!attempt1IsToday && !attempt2IsToday) {
					console.log(
						`⚠️ Neither follow-up attempt date is today - skipping deal ${dealInfo.deal.id} (skipping expensive fetch)`,
					);
					continue; // Skip silently - date not today
				}

				// Check Trial stage for attempt 2 BEFORE fetching
				if (attempt2IsToday && dealInfo.deal.stage_id === 24) {
					console.log(
						`⚠️ Deal ${dealInfo.deal.id} is in Trial stage (stage_id: 24) and attempt 2 date matches - skipping (deal is already a customer)`,
					);
					continue;
				}
			}
			...
			// Fetch deal details just-in-time (only if date check passed or not a follow_up stage)
			...
			if (Either.isRight(dealDataResult)) {
				...
				if (
					...
					// Dates already validated before fetch - re-check to determine which stage to process
					...
					// Re-check dates to determine which stage to process
					const attempt1IsToday = isDateToday(attempt1Date);
					const attempt2IsToday = isDateToday(attempt2Date);
					...
					// Process follow-up deals: create EmailSequence records
					...
					for (const [index, account] of usaRows.entries()) {
						const now = new Date();
						console.log(
							`[${index + 1}/${usaRows.length}] Creating sequence for ${account.id}`,
						);
					...
					// Handle qualifizierung and other deals (no date checking required)
					console.log(
						`Processing ${data.followupStage} deal ${data.deal.id} (no date check required)`,
					);
					...
					for (const [index, account] of usaRows.entries()) {
						...
						console.log(
							`idsequencecountry: ${idsequencecountry} (${data.followupStage})`,
						);
